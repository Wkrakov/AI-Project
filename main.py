...
import socket
import requests # Добавляем для проверки интернет-соединения
...
def load_ocr_model():
    ...
    try:
        ...
        st.error(f"Не удалось загрузить OCR модель. Возможно, проблема с подключением к Интернету. Ошибка: {e}")
        return None
...
def is_connected():
    ...
    try:
        # Попытка установить соединение с известным сервером (Google DNS)
        socket.create_connection(("8.8.8.8", 53), timeout=5)
        # Попытка сделать простой HTTP запрос к Hugging Face
        ...
        st.warning(f"Не удалось проверить интернет-соединение: {e}")
        return False # Если проверка не удалась, лучше считать, что соединения нет
...
if not is_connected():
    st.warning("Приложению требуется подключение к Интернету для загрузки OCR модели. Пожалуйста, проверьте ваше соединение.")
    st.stop() # Останавливаем выполнение, если нет интернета
